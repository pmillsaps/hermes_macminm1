...
def vault_files_by_priority():
    """Yield (relative_path, absolute_path) in priority order."""
...
def get_timing_summary(state=None):
    """Return timing summary from batch history."""
    if state is None:
        state = load_state()
...
def main():
    if len(sys.argv) < 2:
        print("Usage: wiki-vault-ingest.py {batch|record|summary|status} [args...]")
        sys.exit(1)
    ...
    if cmd == "batch":
        ...
        recent = sorted(ingested.items(), key=lambda x: x[1].get("last_processed", ""), reverse=True)[:5]
        if recent:
            print("\nRecently processed:")
